...
from app.config import SCAN_MAX_Z_CAP_DEFAULT, SCAN_MAX_Z_CAP_HARD_MAX
...
def resolve_scan_cap(db: Path) -> int:
    """读取 scan_max_z_cap 设置；缺失或写坏时回退安全默认值。"""
    raw = get_setting(db, "scan_max_z_cap", "")
    try:
        cap = int(str(raw).strip())
    except (TypeError, ValueError):
        return SCAN_MAX_Z_CAP_DEFAULT
    if cap < 0 or cap > SCAN_MAX_Z_CAP_HARD_MAX:
        return SCAN_MAX_Z_CAP_DEFAULT
    return cap
...
def run_scan(db: Path, data_dir: Path, slug: str, scheme: str, max_z: int) -> dict:
    ...
    if not layer:
        raise KeyError(slug)
    requested_max_z = max_z
    cap = resolve_scan_cap(db)
    # 实际扫描 zoom 同时受请求值、全局 cap、图层自身 max_z 约束。
    ...
    with session(db) as conn:
        cur = conn.execute(
            """INSERT INTO scan_runs
                   (layer_slug, scheme, requested_max_z, max_z_cap, max_z,
                    status, total, missing, y_flip, meta_missing, created_at, finished_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                slug,
                scheme,
